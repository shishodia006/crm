...
export default function TemplatesPage() {
  ...
  const syncWa = async (syncChannel, accountId, silent = false) => {
    ...
    try {
      ...
      // Auto-sync used to fail silently here (console.error only) — nobody
      // ever saw it, so a broken key could sit unnoticed indefinitely. Now it
      // surfaces as a banner on the page itself instead of needing someone to
      // open devtools or manually click Sync to discover the real reason.
  ...
  return (
    <>
      ...
      <div className="d-flex align-items-center mb-4 gap-2 flex-wrap">
        ...
        <button className="btn btn-outline-success btn-sm" disabled={syncingWa} onClick={() => syncWa('whatsapp', waAccountId)}>
        ...
        <button className="btn-crm-outline" disabled={syncingRcs} onClick={() => syncWa('rcs', rcsAccountId)}>
