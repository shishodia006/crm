...
export async function show(req, res) {
  ...
  const [timeline, campaigns, agents, sources, enrollments, scoreHistory, tags] = await Promise.all([
    ...
    q('SELECT t.id,t.name,t.color FROM tags t JOIN lead_tags lt ON lt.tag_id=t.id WHERE lt.lead_id=? ORDER BY t.name', [leadId])
  ]);
  ok(res, { lead, timeline, campaigns, agents, sources, enrollments, scoreHistory, tags });
}
