...
export async function dealsIndex(req, res) {
  ...
  const total = Number(await scalar('SELECT COUNT(*) FROM deals WHERE company_id=?', [req.companyId]));
  const deals = await q(
    `SELECT d.*, l.name AS lead_name, ps.name AS stage_name, u.name AS agent_name
     ...
     WHERE d.company_id=? ORDER BY d.created_at DESC LIMIT ${perPage} OFFSET ${offset}`,
    [req.companyId]
  );
...
export async function dealsUpdate(req, res) {
  ...
  const deal = await one('SELECT id FROM deals WHERE id=? AND company_id=? LIMIT 1', [Number(req.params.id), req.companyId]);
  if (!deal) return fail(res, 'Deal not found.', 404);
  await updateById('deals', deal.id, data);
...
export async function moveStage(req, res) {
  ...
  const deal = await one('SELECT id,stage_id FROM deals WHERE id=? AND company_id=? LIMIT 1', [dealId, req.companyId]);
  if (!deal) return fail(res, 'Deal not found.', 404);
  ...
  await run('UPDATE deals SET stage_id=? WHERE id=? AND company_id=?', [stageId, dealId, req.companyId]);
...
export async function addTask(req, res) {
  const deal = await one('SELECT id FROM deals WHERE id=? AND company_id=? LIMIT 1', [Number(req.params.id), req.companyId]);
  if (!deal) return fail(res, 'Deal not found.', 404);
  await run('INSERT INTO tasks (company_id,title,deal_id,assigned_to,priority,due_at,created_by) VALUES (?,?,?,?,?,?,?)', [
    req.companyId, req.body.title, deal.id,
...
export async function addMeeting(req, res) {
  const deal = await one('SELECT lead_id FROM deals WHERE id=? AND company_id=? LIMIT 1', [Number(req.params.id), req.companyId]);
  if (!deal) return fail(res, 'Deal not found.', 404);
  await run(
...
export async function markWon(req, res) {
  ...
  await transaction(async (conn) => {
    const deal = await one('SELECT * FROM deals WHERE id=? AND company_id=? LIMIT 1', [dealId, req.companyId], conn);
    ...
    await run("UPDATE lead_enrollments SET status='converted', exit_reason='deal_won', completed_at=NOW() WHERE lead_id=? AND status IN ('active','paused')", [deal.lead_id], conn);
...
export async function markLost(req, res) {
  ...
  const deal = await one('SELECT id FROM deals WHERE id=? AND company_id=? LIMIT 1', [dealId, req.companyId]);
  if (!deal) return fail(res, 'Deal not found.', 404);
  await run('UPDATE deals SET lost_at=NOW(), lost_reason=?, stage_id=?, won_at=NULL, actual_close=CURDATE() WHERE id=? AND company_id=?', [req.body.reason || '', lostStage.id, dealId, req.companyId]);
  await run("UPDATE leads l JOIN deals d ON d.lead_id=l.id SET l.status='lost', l.lost_at=NOW(), l.lost_reason=?, l.won_at=NULL WHERE d.id=? AND d.company_id=?", [req.body.reason || '', dealId, req.companyId]);
